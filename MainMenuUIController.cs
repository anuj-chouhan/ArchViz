...
{
    //Collecting all the ui elements
    [Header("UI Elements")]
    ...
    public UnityEvent OnPlayerModeChange;       //Event when player mode changes (First person mode and Drone mode)
    public static MainMenuUIController instance;    //Singleton

    private void Awake()
    {
        if (instance != null)
        {
            Debug.LogError("Signleton Error Here"); //Ensuring only one instance exists
        }
